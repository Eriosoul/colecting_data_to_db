...
class MusicFromWeb:
            ...
            for row in rows:
                ...
                if len(columns) >= 5:
                    info_content = MusicEntry(
                        ...
                        pais=columns[4].text.strip().split()[0]
                    )
    ...
    def get_json_data(self):
        with open('templates/paises.json') as json_file:
            json_data = json.load(json_file)
            return json_data
def main_request():
    m: MusicFromWeb = MusicFromWeb()
    json_data = m.get_json_data()  # Obtener datos JSON
    data_content = m.get_info(json_data)
